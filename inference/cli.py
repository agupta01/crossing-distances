...
from inference.utils import create_logger, app, filter_coordinates, fuzzy_search_optimized, get_from_volume, main_scratch
...
def download_scratch_to_temp_dir(filepath, env, conda_env='ox'):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if conda_env:
            if not conda_env:
                raise EnvironmentError("CONDA_DEFAULT_ENV is not set. Make sure you are running inside a conda environment.")
...
def get_file_from_all_envs(envs_path: str, filename: str, destination_dir: str):
    ...
    for _, env in tqdm(envs.iterrows(), total=len(envs)):
        ...
        subprocess.run(f"uv run modal volume get -e {env_code} scratch {filename} {destination_dir}/")
...
def verify(env_name: str, logger: Logger):
    ...
    crosswalks_df = pd.read_csv(get_from_volume("scratch", "intersection_coordinates.csv", environment_name=env_name))
    crosswalks_vol = modal.Volume.lookup(f"crosswalk-data-{env_name}", environment_name=env_name)
    filenames = list(map(lambda x: x.path, crosswalks_vol.listdir("/")))
    missing_coords: pd.DataFrame = filter_coordinates(crosswalks_df, env_name, -1, logger, return_df=True)
    if len(missing_coords) != 0:
        missing_coords["best_match"] = missing_coords["filename"].apply(lambda x: fuzzy_search_optimized(x, filenames))
        missing_coords["error"] = missing_coords.apply(lambda row: sum(1 if row["filename"][i] != row["best_match"][i] else 0 for i in range(len(row["filename"]))), axis=1)
        missing_coords.to_csv(f"/scratch/missing_intersections_{env_name}.csv", index=False)
        logger.error(f"[MISSING INTERSECTIONS] {len(missing_coords)} intersections missing in {env_name}. Missing coordinates and closest matches are saved at /scratch/missing_intersections_{env_name}.csv")
...
def run(envs_path: str, function: str, subset: str | None = None):
    ...
    for i, env in envs.iterrows():
        env_code = env["Code"].lower()
        calls.append(run_single_city(env_code, function, logger, place=f"{env['Title']}, {env['State']}"))
...
def run_single_city(env_name: str, function: str, logger: Logger, place: str) -> FunctionCall:
    ...
    f = modal.Function.lookup(
        APP_NAME, function, environment_name=env_name
    )
    if function == "main":
...
def main():
    ...
    # Deploy command
    deploy_parser = subparsers.add_parser('deploy', help='Deploy functions in file to all environments')
    deploy_parser.add_argument('--envs-path', type=str, required=True, help='Path to environments file. Should contain a column "Code" with the environment names.')
    deploy_parser.add_argument('--functions-file', type=str, required=True, help='Path to functions Python file.')
    deploy_parser.add_argument('--subset', type=str, required=False, help='Subset of environments to run the function in. Comma separated list of environment names. If not specified, runs in all environments.')

    # Run command
    run_parser = subparsers.add_parser('run', help='Run a function in all or selected environments')
    run_parser.add_argument('--envs-path', type=str, required=True, help='Path to environments file. Should contain a column "Code" with the environment names.')
    run_parser.add_argument('--function', type=str, required=True, help='Function to run.')
    run_parser.add_argument('--subset', type=str, required=False, help='Subset of environments to run the function in. Comma separated list of environment names. If not specified, runs in all environments.')

    # Get file command
    get_parser = subparsers.add_parser('get', help='Get a file from all environments')
    get_parser.add_argument('--envs-path', type=str, required=True, help='Path to environments file. Should contain a column "Code" with the environment names.')
    get_parser.add_argument('--filename', type=str, required=True, help='Filename to get from all environments.')
    get_parser.add_argument('--destination-dir', type=str, required=True, help='Destination directory to save the file.')
